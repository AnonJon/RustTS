...
pub fn build_map_with_params(seed: u64, params: &MapParams) -> MapConfig {
    ...
    if params.connect_players {
        generate_connections(&mut grid, &players, w, h);
    }
...
fn generate_terrain_clumps(
    ...
    // Dirt patches (generated first, sequential in script order)
    ...
    // Dirt fringe around forests: grass tiles adjacent to forest get a chance
    // to become dirt, simulating natural forest-edge bare earth
    ...
    for x in 1..w - 1 {
        for y in 1..h - 1 {
            ...
            let has_forest_neighbor = NEIGHBORS_4.iter().any(|&(dx, dy)| {
                let nx = x + dx;
                let ny = y + dy;
                nx >= 0
                    && nx < w
...
fn generate_connections(grid: &mut [Vec<Tile>], players: &[PlayerStart], w: i32, h: i32) {
    if players.len() < 2 {
        return;
    }
    ...
    for player in players.iter().skip(1) {
        if !bases_connected(grid, first, player.position, w, h) {
            carve_corridor(grid, first, player.position, w, h);
        }
    }
}
...
fn generate_objects(
    ...
    place_central_resources(rng, cx, cy, w, h, grid, &mut clusters, &occupied);
    clusters
}
...
fn place_central_resources(
    ...
    if let Some(positions) =
        find_cluster_in_ring(rng, (cx, cy), 0.0, 5.0, 2, 2, false, false, &occ, w, h, grid)
    {
        let center = cluster_center(&positions);
        occ.push(center);
    ...
    if let Some(positions) =
        find_cluster_in_ring(rng, (cx, cy), 0.0, 7.0, 2, 1, false, false, &occ, w, h, grid)
    {
        clusters.push(ResourceCluster {
            positions,
...
fn find_cluster_in_ring(
    ...
    for pass in 0..2u32 {
        ...
        for i in 0..N {
            ...
            let center = cluster_center(&positions);
            if !is_separated(center, occupied) {
                continue;
            }
...
fn cluster_center(positions: &[(i32, i32)]) -> (i32, i32) {
    let n = positions.len() as i32;
    if n == 0 {
        return (0, 0);
    }
    let sx: i32 = positions.iter().map(|&(x, _)| x).sum();
    let sy: i32 = positions.iter().map(|&(_, y)| y).sum();
    (sx / n, sy / n)
}

fn is_separated(center: (i32, i32), occupied: &[(i32, i32)]) -> bool {
    let sep_sq = MIN_CLUSTER_SEP * MIN_CLUSTER_SEP;
    occupied
        .iter()
        .all(|&(ox, oy)| (center.0 - ox).pow(2) + (center.1 - oy).pow(2) >= sep_sq)
}
